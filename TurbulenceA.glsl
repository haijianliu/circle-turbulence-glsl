...
float smoothDist(in float left, in float right, in float x){
	return smoothstep(left ,left + left * 0.01, x) - smoothstep(right - 0.01 * right, right, x);
}
...
float ring(in vec2 pos, in float delay){
	...
	float r = length(pos) + 0.01 * smoothNormal(0.0, 0.5 * PI, a) * cos(12.0 * sin(iGlobalTime) * (a + delay));
